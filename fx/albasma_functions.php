...
function getArticles($content ,$title_selector , $container_selector, $cs1, $cs2 ,$cs3,$time_selector){
	$articles =[];	
	foreach($content->find($container_selector) as $key => $title_list)
	{
		...
		{			
			...
			foreach ($content->find($cs1) as $text) 
			{
				$data = $data.$text->plaintext;
			}	
			foreach ($content->find($cs2) as $text) 
			{
				$data = $data.$text->plaintext;
			}
			foreach ($content->find($cs3) as $text) 
			{
				$data = $data.$text->plaintext;
			}	
